...
describe('GET /api/dashboard/terrain-clusters/:sn', () => {
  it('400 bij een ongeldige sn (encoded slash — anders normaliseert Express de route weg)', async () => {
    const res = await request(server).get('/api/dashboard/terrain-clusters/..%2f..%2fx');
    expect(res.status).toBe(400);
  });

  it('lege lijst zonder clusters', async () => {
    const res = await request(server).get('/api/dashboard/terrain-clusters/LFIN0000000010');
  ...
  it('levert het volledige cluster-shape, className = model zonder override', async () => {
    ...
    const res = await request(server).get(`/api/dashboard/terrain-clusters/${SN}`);
    expect(res.status).toBe(200);
    expect(res.body.clusters).toEqual([{
  ...
  it('correctie op één tegel geldt voor het hele samengevoegde object', async () => {
    ...
    const lijst = await request(server).get(`/api/dashboard/terrain-clusters/${SN2}`);
    ...
    const na = await request(server).get(`/api/dashboard/terrain-clusters/${SN2}`);
...
describe('GET /api/dashboard/terrain-crops/:sn/:file', () => {
  it('400 bij een ongeldige sn (encoded slash)', async () => {
    const res = await request(server).get('/api/dashboard/terrain-crops/..%2f..%2fx/1,2.jpg');
    expect(res.status).toBe(400);
  });

  it('400 bij een bestandsnaam buiten de whitelist (path-traversal guard)', async () => {
    const res = await request(server).get(`/api/dashboard/terrain-crops/${SN}/..%2f..%2fetc%2fpasswd`);
    expect(res.status).toBe(400);
  });

  it('404 als de foto niet bestaat', async () => {
    const res = await request(server).get(`/api/dashboard/terrain-crops/${SN}/9,9.jpg`);
  ...
  it('accepteert tegel-sleutels met t-prefix (404, niet 400)', async () => {
    ...
    const res = await request(server).get(`/api/dashboard/terrain-crops/${SN}/t0,-7.jpg`);
    expect(res.status).toBe(404);
  });

  it('400 bij een letter die geen t-prefix is', async () => {
    const res = await request(server).get(`/api/dashboard/terrain-crops/${SN}/x0,-7.jpg`);
    expect(res.status).toBe(400);
  });
  ...
  it('200 met de jpeg-bytes als de foto bestaat', async () => {
    ...
    const res = await request(server).get(`/api/dashboard/terrain-crops/${SN}/1,2.jpg`)
...
describe('POST /api/dashboard/terrain-clusters/:sn/:key/override', () => {
  ...
  it('400 bij een ongeldige sn (encoded slash)', async () => {
    const res = await request(server).post('/api/dashboard/terrain-clusters/..%2f..%2fx/1,2/override').send({ className: 'tree' });
    expect(res.status).toBe(400);
  });

  it('400 bij een className die niet in LABELS voorkomt', async () => {
    const res = await request(server).post(`/api/dashboard/terrain-clusters/${SN}/1,2/override`).send({ className: 'unicorn' });
    expect(res.status).toBe(400);
  });

  it('404 als het cluster niet bestaat', async () => {
    const res = await request(server).post(`/api/dashboard/terrain-clusters/${SN}/9,9/override`).send({ className: 'tree' });
    expect(res.status).toBe(404);
  });

  it('200 en zet de override', async () => {
    const res = await request(server).post(`/api/dashboard/terrain-clusters/${SN}/1,2/override`).send({ className: 'tree' });
    expect(res.status).toBe(200);
  ...
  it('200 met className:null wist de override', async () => {
    terrainClusterRepo.setOverride(SN, '1,2', 'tree');
    const res = await request(server).post(`/api/dashboard/terrain-clusters/${SN}/1,2/override`).send({ className: null });
    expect(res.status).toBe(200);
  ...
  it("'__none__'-override verwijdert het object uit de lijst maar bewaart de rij", async () => {
    ...
    const lijst = await request(server).get(`/api/dashboard/terrain-clusters/${SN3}`);
  ...
  it('handmatig object toevoegen: verschijnt in de lijst met m-sleutel en override', async () => {
    ...
    const lijst = await request(server).get(`/api/dashboard/terrain-clusters/${SN4}`);
  ...
  it('handmatig object overleeft de wees-opruiming', async () => {
    ...
    await request(server).post(`/api/dashboard/terrain-clusters/${SN5}/add`)
  ...
  it('add weigert onbekende klasse en rare coords', async () => {
    ...
    await request(server).post(`/api/dashboard/terrain-clusters/${SN6}/add`)
  ...
  it('custom model: upload, lijst, serveren, koppelen aan object', async () => {
    ...
    const lijst = await request(server).get('/api/dashboard/terrain-models');
    expect(lijst.body.models).toContain('mijn-stoel-.glb');

    const served = await request(server).get('/api/dashboard/terrain-models/mijn-stoel-.glb');
    expect(served.status).toBe(200);

    await request(server).post(`/api/dashboard/terrain-clusters/${SN7}/add`)
      .send({ x: 1, y: 1, className: 'garden chair', height: 1.2 });
    const key = 'm10,10';
    const koppel = await request(server)
      .post(`/api/dashboard/terrain-clusters/${SN7}/${key}/model`)
      .send({ file: 'mijn-stoel-.glb' });
    expect(koppel.status).toBe(200);

    const cl = await request(server).get(`/api/dashboard/terrain-clusters/${SN7}`);
  ...
  it('model-upload weigert niet-GLB en traversal-namen', async () => {
    await request(server).post('/api/dashboard/terrain-models/upload?name=x')
      .set('Content-Type', 'application/octet-stream')
      .send(Buffer.from('nietglb-data-hier')).expect(400);
    await request(server).get('/api/dashboard/terrain-models/..%2fx.glb').expect(400);
  });
  ...
  it('re-add op dezelfde plek werkt als maat-update en behoudt model_file', async () => {
    ...
    const glb = Buffer.concat([Buffer.from('glTF'), Buffer.alloc(12)]);
    await request(server).post('/api/dashboard/terrain-models/upload?name=resizetest')
      .set('Content-Type', 'application/octet-stream').send(glb);
    await request(server).post(`/api/dashboard/terrain-clusters/${SN8}/add`)
      .send({ x: 2, y: 2, className: 'tree', size: 1, height: 0.5 });
    await request(server).post(`/api/dashboard/terrain-clusters/${SN8}/m20,20/model`)
      .send({ file: 'resizetest.glb' });
    // maat aanpassen via re-add op exact dezelfde coords
    await request(server).post(`/api/dashboard/terrain-clusters/${SN8}/add`)
      .send({ x: 2, y: 2, className: 'tree', size: 3, height: 2 });
    const cl = await request(server).get(`/api/dashboard/terrain-clusters/${SN8}`);
  ...
  it('display-overrides zetten en resetten', async () => {
    ...
    await request(server).post(`/api/dashboard/terrain-clusters/${SN9}/add`)
    ...
    let cl = await request(server).get(`/api/dashboard/terrain-clusters/${SN9}`);
    ...
    // reset
    await request(server).post(`/api/dashboard/terrain-clusters/${SN9}/m10,10/display`)
      .send({ size: null, height: null, zOffset: null });
    cl = await request(server).get(`/api/dashboard/terrain-clusters/${SN9}`);
  ...
  it('model hernoemen: bestand hernoemd en objectverwijzing bijgewerkt', async () => {
    ...
    const glb = Buffer.concat([Buffer.from('glTF'), Buffer.alloc(12)]);
    await request(server).post('/api/dashboard/terrain-models/upload?name=lelijkenaam-x9')
      .set('Content-Type', 'application/octet-stream').send(glb);
    await request(server).post(`/api/dashboard/terrain-clusters/${SN10}/add`)
      .send({ x: 1, y: 1, className: 'tree' });
    await request(server).post(`/api/dashboard/terrain-clusters/${SN10}/m10,10/model`)
      .send({ file: 'lelijkenaam-x9.glb' });

    const ren = await request(server)
    ...
    const cl = await request(server).get(`/api/dashboard/terrain-clusters/${SN10}`);
  ...
  it('x/y-verschuiving verschuift de geometrie in de GET-respons', async () => {
    ...
    await request(server).post(`/api/dashboard/terrain-clusters/${SN11}/add`)
      .send({ x: 2, y: 2, className: 'tree', size: 1 });
    await request(server).post(`/api/dashboard/terrain-clusters/${SN11}/m20,20/display`)
      .send({ xOffset: 1.5, yOffset: -0.5 });
    const cl = await request(server).get(`/api/dashboard/terrain-clusters/${SN11}`);
