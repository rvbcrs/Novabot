...
// request een nieuwe efemere poort openen; dit bestand doet er achttien en de
// suite draait bestanden parallel, waardoor er af en toe een request bij een
// vreemd proces uitkwam en de test na 5 s afkapte. Dat heeft vandaag een
// release gekost. Eén server, één poort, geen race.
...
it.each(['import-zip', 'upload-zip'])('%s preserves nonzero slots from a real ZIP and never replaces existing maps', async (endpoint) => {
  ...
  try {
    ...
    const r = await request(server).post(`/api/dashboard/maps/${sn}/${endpoint}`).send(body);
    ...
    const repeated = await request(server).post(`/api/dashboard/maps/${sn}/${endpoint}`).send(body);
...
describe('mowing area limit at both command entry points (#114)', () => {
  ...
  it.each([
    ...
    const r = await request(server).post('/api/dashboard/command/LFIN_AREA_LIMIT')
  ...
  it.each([
    ...
    const r = await request(server).post('/api/dashboard/extended/LFIN_AREA_LIMIT')
  ...
  it('preserves all five supported zones in the command and custom routes', async () => {
    ...
    const normal = await request(server).post('/api/dashboard/command/AREA_LIMIT').send({ command });
    expect(normal.status).toBe(200);
    expect(publishToDevice).toHaveBeenCalledWith('AREA_LIMIT', command);
    const extendedCommand = { mow_zone: { map: 'map0', area: 11111 } };
    const extended = await request(server).post('/api/dashboard/extended/AREA_LIMIT').send(extendedCommand);
  ...
  it.each([
    ...
    const r = await request(server).post('/api/dashboard/command/AREA_LIMIT').send({ command });
    expect(r.status).toBe(200);
    expect(publishToDevice).toHaveBeenCalledWith('AREA_LIMIT', command);
  });
...
describe('rand-dag watcher disarm via dashboard-routes', () => {
  ...
  it('POST /stop-navigation/:sn ontwapent de watcher', async () => {
    ...
    armViaRunner(SN);
    const r = await request(server).post(`/api/dashboard/stop-navigation/${SN}`);
    expect(r.status).toBe(200);
  ...
  it('DELETE /schedules ontwapent de watcher van dat schema', async () => {
    ...
    armViaRunner(SN);
    const r = await request(server).delete(`/api/dashboard/schedules/${SN}/sched-${SN}`);
    expect(r.status).toBe(200);
    expect(__getPendingEdgeForTest().has(SN)).toBe(false);
  });
