...
### Aanvullende live blokkade: GPS-sprong op stilstaand dock

Na de beta-uitrol bevestigde de gebruiker dat .244 niet had bewogen. De ontvangerpositie sprong toch 1,434 m in 0,205 s, met RTK Fixed voor en na. Een gelijktijdige capture bevestigt dat raw BESTPOS, geparste BESTPOS en `gps_raw` overeenkomen. De uitgegeven voertuigpose bleef vrijwel staan; de afwijking tegenover raw GNSS, de ongewijzigde oorsprong en de volledige antenne-TF bedroeg circa 1,426 m. De kanaalcorrectie is niet toegepast.

De live lokalisatiebinary is bytegelijk aan de lokale stock-v6.0.2-binary (SHA256 `c85420918b8fd718e8aedda6d7a08efec988ddf66af1307f562d20e1f5b3484f`). Lokale decompilatie bevestigt de logmelding `JUMP_FIXED_SELF_CONTINUAL`: `updateOdomToMapTf` gebruikt in deze toestand de huidige GNSS-positie plus een intern voorspeld anker minus het bij de sprong opgeslagen GNSS-anker. Een raw/fused-verschil bewijst daardoor op zichzelf geen fysieke verschuiving van de lokale kaart.

Die bescherming is niet blijvend gegarandeerd. `updateRTKContinuousStatus` kan haar na gereden afstand, voldoende satellieten en gunstige HDOP opheffen terwijl het positieverschil nog groot is. De stock-code waarschuwt hierbij zelf voor een mogelijke lokalisatiefout. De gebruikte afstandsgrens wordt dynamisch berekend en begrensd tussen 1,2 en 10 m; de aanwezige parameter `max_move_dis_fixed_jump: 20` mag dus niet als gegarandeerde veilige rijafstand worden geïnterpreteerd. `loadUtmOriginInfoCallback` roept bovendien `initTaskData` aan en reset daarmee de jump-toestand voordat het de oorsprong inleest. Een herankering is geen onschuldige wijziging van alleen een bestand.

Vervolg onder `Novabot-55f.22` en `.19`: de oorzaak van de GNSS-sprong onderscheiden van de tijdelijke compensatie, en dit meenemen in framevalidatie en herankering. De bestaande Linux-telemetrie bevat geen RTCM-basispositie; basis versus rover is hiermee nog niet bewezen. Zonder die onderbouwing worden de 5cm-grens, oorsprong en opgeslagen dockpose niet aangepast om het herstel alsnog te laten slagen. Deze analyse heeft geen apparaatinstellingen of firmware veranderd en geen beweging veroorzaakt.
