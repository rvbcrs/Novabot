/**
 * Map screen — lightweight SVG-based map showing mower position,
 * charger, map polygons, and GPS trail. No native maps dependency.
 */
...
const INNER_PADDING = 20; // padding inside the SVG
...
export default function MapScreen() {
  ...
  const mower = useMemo(
    () => [...devices.values()].find((d) => d.deviceType === 'mower') ?? null,
    [devices],
  );
  const charger = useMemo(
    () => [...devices.values()].find((d) => d.deviceType === 'charger') ?? null,
    [devices],
  );

  // Mower GPS from live sensors
  ...
  // Compute overall bounds from all data
  const bounds = useMemo(() => {
    let b: Bounds | null = null;
    for (const m of maps) {
      b = expandBounds(b, computeBounds(m.mapArea));
    }
    if (trail.length > 0) {
      b = expandBounds(b, computeBounds(trail));
    }
    ...
    if (b) {
      ...
      b = {
        ...
        minLng: b.minLng - lngPad, maxLng: b.maxLng + lngPad,
      };
    }
    return b;
        ...
        {!loading && !hasData && !bounds && (
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptySubtitle}>
              {connected ? 'Create a map from the mower to see it here.' : 'Connecting to server...'}
            </Text>
          </View>
        )}
          ...
          <View style={styles.mapContainer}>
            <Svg width={MAP_SIZE} height={MAP_SIZE} viewBox={`0 0 ${MAP_SIZE} ${MAP_SIZE}`}>
              ...
              {Array.from({ length: 5 }, (_, i) => {
                ...
                return (
                  <G key={`grid-${i}`}>
                    <Line x1={INNER_PADDING} y1={pos} x2={MAP_SIZE - INNER_PADDING} y2={pos}
                      stroke="rgba(255,255,255,0.04)" strokeWidth={1} />
                    <Line x1={pos} y1={INNER_PADDING} x2={pos} y2={MAP_SIZE - INNER_PADDING}
                      stroke="rgba(255,255,255,0.04)" strokeWidth={1} />
                  </G>
                );
              })}
              ...
              {maps.map((m) => {
                ...
                const pts = m.mapArea
                  .map((p) => gpsToSvg(p, bounds, MAP_SIZE, INNER_PADDING))
                  .map((p) => `${p.x},${p.y}`)
                  .join(' ');
                return (
                  <SvgPolygon
              ...
              {chargerGps && (() => {
                ...
                return (
                  <G>
                    ...
                    <Path
                      d={`M${cp.x - 2.5} ${cp.y - 4} L${cp.x + 2.5} ${cp.y - 4} L${cp.x + 1} ${cp.y} L${cp.x + 3} ${cp.y} L${cp.x - 1} ${cp.y + 5} L${cp.x} ${cp.y + 1} L${cp.x - 2} ${cp.y + 1} Z`}
                      fill="#f59e0b"
                    />
                  </G>
                );
              ...
              {/* Mower marker with heading */}
              {mowerGps && (() => {
                const mp = gpsToSvg(mowerGps, bounds, MAP_SIZE, INNER_PADDING);
                const rad = ((heading - 90) * Math.PI) / 180;
                const arrowLen = 14;
                const ax = mp.x + Math.cos(rad) * arrowLen;
                const ay = mp.y + Math.sin(rad) * arrowLen;
                return (
                  <G>
                    {/* Heading direction line */}
                    <Line x1={mp.x} y1={mp.y} x2={ax} y2={ay}
            ...
            {mowerGps && (
              <View style={styles.chip}>
                ...
                <Text style={styles.chipText}>
                  {mowerGps.lat.toFixed(5)}, {mowerGps.lng.toFixed(5)}
                </Text>
              </View>
            )}
...
const styles = StyleSheet.create({
  ...
  scroll: { flex: 1, padding: MAP_PADDING },
  header: {
  ...
  emptySubtitle: { fontSize: 15, color: colors.textDim, textAlign: 'center' },
  mapContainer: {
    backgroundColor: colors.card, borderRadius: 20,
    borderWidth: 1, borderColor: colors.cardBorder,
    padding: 0, overflow: 'hidden', alignItems: 'center',
  },
  legend: {
