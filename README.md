...
| `SIMULATE_STOCK_FIRMWARE` | _(empty)_ | Test switch: comma-separated mower SNs the server treats as **stock** firmware (all custom-only features refused with 409, reported version shown as `6.0.2-simulated-stock`). For testing the stock experience on a custom-firmware mower only. |
