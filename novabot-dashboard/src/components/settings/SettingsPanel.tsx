  ...
  return (
    <div className="p-4 space-y-5">
      ...
      <ToggleRow
        ...
        description={t('settings.headlightDesc')}
        active={headlightOn}
        disabled={disabled}
...
function ToggleRow({ icon: Icon, label, description, active, disabled, onToggle }: {
  ...
  return (
    <div className="flex items-center justify-between gap-3">
      <div className="flex items-center gap-2 min-w-0">
        <Icon className={`w-4 h-4 flex-shrink-0 ${active ? 'text-yellow-300' : 'text-gray-500'}`} />
      ...
      <button
        onClick={onToggle}
        disabled={disabled}
        className={`relative flex-shrink-0 w-10 h-5 rounded-full transition-colors disabled:opacity-30 disabled:cursor-not-allowed ${
