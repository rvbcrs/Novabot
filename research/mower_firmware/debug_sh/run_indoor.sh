...
start)
  ...
  PIDS=`ps -ef|grep "iox-roudi"|grep -v grep `
  if [ "$PIDS" == "" ]; then
  echo "Start iox-roudi!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
  export RMW_IMPLEMENTATION=rmw_cyclonedds_cpp
  export CYCLONEDDS_URI=file:///root/novabot/shm_config/shm_cyclonedds.xml
  rm /root/novabot/log/iox.log
  nohup iox-roudi  -c /root/novabot/shm_config/shm_ioxroudi.toml --log-level verbose > /root/novabot/log/iox.log &
  unset RMW_IMPLEMENTATION
  unset CYCLONEDDS_URI
  else
  echo "Iox-roudi is already started---------------"
  fi
  killall -q -15 python
  killall -q -15 ros2
 ...
 export CYCLONEDDS_URI=file:///root/novabot/shm_config/shm_cyclonedds.xml
  ...
  sleep 5s
  ros2 launch aruco_localization aruco_localization_shm.launch.py  &
  sleep 5s
  unset RMW_IMPLEMENTATION
  unset CYCLONEDDS_URI
  ros2 launch chassis_control novabot_chassis_start.launch.py   use_sim_time:=False enable_map_as_odom:=True &
  # ros2 launch robot_combination_localization localization_only.launch.py use_sim_time:=False &
  ros2 launch nav2_single_node_navigator nav2_single_node_navigator.launch.py use_sim_time:=False &
