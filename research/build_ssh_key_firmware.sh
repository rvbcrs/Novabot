...
# Root wachtwoord NIET veranderen — we willen het originele wachtwoord uit /etc/shadow lezen
# echo "root:${SSH_PASSWORD}" | chpasswd 2>/dev/null

# Voeg SSH authorized key toe (inloggen zonder wachtwoord)
mkdir -p /root/.ssh
chmod 700 /root/.ssh
SSH_PUBKEY="ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIAiGv0ybUqxNXeKnAPND2ZcDBsZ69GFizvw47jy5Hqfx rvbcrs@MacBook-Air.local"
grep -qF "AAAAIAiGv0yb" /root/.ssh/authorized_keys 2>/dev/null || echo "\$SSH_PUBKEY" >> /root/.ssh/authorized_keys
chmod 600 /root/.ssh/authorized_keys
echo "SSH authorized key added (password NOT changed)" >> \$path/start_service.log
# ============================================================
SSHEOF
