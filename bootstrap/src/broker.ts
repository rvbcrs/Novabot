...
function tryExtractVersion(io: IOServer, sn: string, payload: Buffer): void {
  ...
  try {
    ...
    if (version && typeof version === 'string') {
      ...
      // Detect custom firmware from version string (e.g. "v6.0.2-custom-21")
      const wasCustom = _isCustomFirmware;
      _isCustomFirmware = version.toLowerCase().includes('custom');
      io.emit('mower-version', { version });
      io.emit('mower-firmware-type', { isCustom: _isCustomFirmware });
      console.log(`[MQTT] Mower version: ${version} (${_isCustomFirmware ? 'custom' : 'stock'} firmware)`);
      if (wasCustom !== _isCustomFirmware) {
        console.log(`[MQTT] Firmware type updated: ${_isCustomFirmware ? 'CUSTOM — any MQTT addr allowed' : 'STOCK — mqtt.lfibot.com required'}`);
      }
    }
  } catch {
...
function requestMowerVersion(sn: string): void {
  ...
  setTimeout(() => {
    ...
    // 1. Encrypted: ota_version_info (stock mqtt_node responds to this)
    const command = { ota_version_info: null };
    const encrypted = encryptForDevice(sn, command);
    publishToMower(sn, encrypted);
    // 2. Plaintext: get_system_info on extended topic (custom firmware / mqtt_bridge responds)
    if (_broker) {
      const extTopic = `novabot/extended/${sn}`;
      _broker.publish({ topic: extTopic, payload: Buffer.from(JSON.stringify({ get_system_info: {} })), qos: 0 as 0, retain: false, cmd: 'publish' as const, dup: false }, () => {});
      console.log(`[MQTT] Sent get_system_info to ${extTopic}`);
    }
  }, 3000);
}
...
export function startBroker(io: IOServer): void {
  ...
  broker.on('publish', (packet: { topic: string; payload: Buffer }, client: { id: string } | null) => {
    ...
    const isExtResponse = packet.topic.startsWith('novabot/extended_response/');
    if (!isReceive && !isServerReceive && !isExtResponse) return;
    ...
    if (isExtResponse) {
      // Extended response is plaintext JSON — extract firmware_version from get_system_info_respond
      try {
        const obj = JSON.parse(packet.payload.toString()) as Record<string, unknown>;
        const sysInfo = obj.get_system_info_respond as Record<string, unknown> | undefined;
        if (sysInfo?.firmware_version && typeof sysInfo.firmware_version === 'string') {
          const version = sysInfo.firmware_version;
          if (!_mowerVersion || _mowerVersion !== version) {
            _mowerVersion = version;
            _isCustomFirmware = version.toLowerCase().includes('custom');
            io.emit('mower-version', { version });
            io.emit('mower-firmware-type', { isCustom: _isCustomFirmware });
            console.log(`[MQTT] Mower version (extended): ${version} (${_isCustomFirmware ? 'custom' : 'stock'})`);
          }
        }
      } catch { /* not JSON */ }
      return;
