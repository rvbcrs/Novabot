    ...
    switch (event->Type()) {
        case NewClient_sMQTTEventType: {
            ...
            if (cid.rfind("LFI", 0) == 0) {
                ...
                // get_system_info is sent from main loop (can't publish from onEvent)
            }
            return true;
        }
        ...
        case Public_sMQTTEventType: {
            ...
            if (topic.find("extended_response") != std::string::npos &&
                ...
                if (fvPos != std::string::npos) {
                    // Find opening quote of the value — handles both :"val" and : "val"
                    size_t start = payload.find('"', fvPos + 18);
                    size_t end = (start != std::string::npos) ? payload.find('"', start + 1) : std::string::npos;
...
void sendMowerOtaWithAes(bool useAes) {
    ...
    // Compute MD5 lazily on first OTA send (saves ~18s at boot)
    if (mowerFwMd5.length() == 0) {
        Serial.println("[OTA] Computing MD5 (first time)...");
        mowerFwMd5 = computeMd5(("/" + mowerFwFilename).c_str());
    }
...
void sendChargerOta() {
    ...
    if (chargerFwMd5.length() == 0) {
        Serial.println("[OTA] Computing charger MD5...");
        chargerFwMd5 = computeMd5(("/" + chargerFwFilename).c_str());
    }
